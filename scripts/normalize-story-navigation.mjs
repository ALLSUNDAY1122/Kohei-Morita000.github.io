...
for (const work of works) {
  ...
  const anchor = seriesAnchors[work.series];
  if (!anchor) throw new Error(`${work.id}: シリーズアンカーが未定義です`);
  ...
  const seriesHtml = `<a class="story-nav-link series-index" href="/kyokai-yawa/#${anchor}"><span>シリーズ一覧</span><strong>${escapeHtml(work.series)}</strong></a>`;
  const nextHtml = next
