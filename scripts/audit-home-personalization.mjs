...
for(const token of ['data-personal-continue','data-personal-recent','data-personal-recommend'])if(!index.includes(token))errors.push(`個別化欄がありません: ${token}`);
...
for(const token of ['POSITION_PREFIX','data-personal-library','data-personal-continue','data-personal-recent','data-personal-recommend','story-taxonomy.json','selectRecommendations','status.getReadIds','status.getHistory'])if(!personalization.includes(token))errors.push(`個別化処理がありません: ${token}`);
...
for(const risky of ['sendBeacon','XMLHttpRequest','WebSocket'])if(personalization.includes(risky)||readingStatus.includes(risky))errors.push(`外部送信に使える処理が含まれています: ${risky}`);
...
const simulate=id=>{
  ...
  const scored=works.filter(work=>work.id!==id).map((work,index)=>{
    const shared=(taxonomy[work.id]?.tags||[]).filter(tag=>contextTags.has(tag));
    let score=shared.length*5;
    if(work.series===source.series)score+=2;
    if(Math.abs(minutes(work)-minutes(source))<=2)score+=1;
    if(Math.abs(Number(work.fear)-Number(source.fear))<=1)score+=1;
    return {work,index,score};
  }).sort((a,b)=>b.score-a.score||a.index-b.index);
  const selected=[];const series=new Set();
  for(const item of scored){if(selected.length>=3)break;if(series.has(item.work.series))continue;selected.push(item.work.id);series.add(item.work.series);}
  for(const item of scored){if(selected.length>=3)break;if(!selected.includes(item.work.id))selected.push(item.work.id);}
  return selected;
};
...
const report=['# 境界夜話 再訪者向け個別入口監査','',`- 公開作品: ${works.length}/48`,'- 続きから読む: 保存位置が8〜93％の作品から最新1話','- 最近読んだ作品: 閲覧・読了時刻順で最大3話','- 未読のおすすめ: 題材・シリーズ・時間・恐怖度から最大3話','- 履歴がない初回訪問: 非表示','- 途中位置リンク: `?resume=1`で保存位置へ移動','- 保存方式: ブラウザー端末内localStorage','- 外部送信: なし',`- エラー: ${errors.length}`,`- 警告: ${warnings.length}`,'','## エラー','',...(errors.length?errors.map(error=>`- ${error}`):['- なし']),'','## 警告','',...(warnings.length?warnings.map(warning=>`- ${warning}`):['- なし']),''].join('\n');
