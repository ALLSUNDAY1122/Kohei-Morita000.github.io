...
test('再訪履歴から続き・最近・保存・未読おすすめを分類JSONなしでも重複なく表示する',async({page})=>{
  const now=Date.now();
  await seedStorage(page,{
    [READ_KEY]:JSON.stringify(['MKB-001']),
  ...
  await page.route('**/data/story-taxonomy.json',route=>route.abort('failed'));

  await page.goto('');
  const library=page.locator('[data-personal-library]');
  await expect(library).toHaveAttribute('data-personalization-ready','true');
